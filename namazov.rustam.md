```diff
- в общей сложности:
+ 18/30. Посещайте лекции и делайте практики. И про курсач не забудьте. Вам это пригодится в жизни. Постарайтесь усвоить материал курса.
- мне важно что там у вас там с другими курсами. На теорию вы ответили хорошо. Напрягитесь и в следующий раз жду от вас практику на 100%.
```
# Java Mid-Term Exam

**Student:** ნამაზოვ რუსთამ
...
```
JRE -джава рунтайм енвиромент нужна для запуска програм
JDK - джава девелопмент кит нужна для написание компиляйии програм
JVM -джава виртуал машин обеспечивает плотформонизависимость
```
...
```
без jvm код пришлость бы компенировать под каждую ос отдельно учитывая архетиктуру процессора
```
...
```
позволяет писать один код и работать на любой платформе
```
```diff
-слишком коротко
```
...
```
Статическое поле - это поля пренадлежащее классу а не объекту общая переменная для всех экземпляров
Статический метод - это метод вызываемый через класс а не объект не имеет доступа к не статистичиским полям
```
...
```
для данных и методов которые общии для всех объектов щетчики , константы , служебные функции
```
...
```
позволяет использовать статистический метод без указание класса
```
...
```diff
- жаль что вы код проигнорировали
```
...
```
Абстрактный класс - это класс от которого нельзя создать объект и который служит базой для наследования, содержащий общее поведение
Абстрактный метод - это метод без реализации который обязан быть переопределен дочернем классе
```
...
```
что бы вынести общиек проведенияи свойства в родительский класс , избежать дублирование кода ,задать единый интерфейс для всех наследников и обеспечить расширяемость системы
```

```diff
- очень хорошие ответы на теорку
```
...
```
обстрактный метод должен быть обязательно реализованны в первом же неабстрактном классе наследники, множественное наследование запрещена
```
