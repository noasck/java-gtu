...
```
Инкапсуляция - это ... защита от дураков, дает и запрещает возможность использовать поля методы в разных классах
```

```diff
-ну ок но, второй части определения нет 1/2
```
...
```
Модификаторы доступа - это ограничитель, который может ограничить доступ к определенному полу или методу(классу), или наоборот может дать возможность использовать класс в любом месте

public - публичный модификатор, который доступен по всюду
private - приватный модификатор который доступен только  в род классе
package-private - приватный пакетный модификатор доступ только перед классом в месте где находятся все пакеты
protected - могут использовать наследники
```
...
```diff
- поля тут не private, а protected/public должны быть.
...
```
Конструктор класса в Java - это ...придает начальное значение объекту, если у объекта не указано значение пользователем, то ему присвается начальные значение int=0 и т.д.
Конструктор по умолчанию (default constructor) - это сборщик объекта, который получая инфу из классов и собирает сам объект
```
```diff
- оооо. ну хоть кто-то это написал. слава яйцам
```
...
```
нет деструктора по причине того что он не требуется в джава, вместо него в джава существует сборщик муссора
```

```diff
- я не знаю где вас учили про деструкторы чему-то и сборщик мусора. А вот в си нет GC и деструкторов нет:)
- 0 баллов.
```
...
```
нет, конструктор не наследуется, а вызвать его можно при помощи команды super
```
...
class Printer3D {
 ...
 int objectsPrinted;
	...
	public Printer3D (String Model, int filamentLevel, int powerLevel)
	{
	 ...
	 public void printobject (int filamentneeded,int powerneeds){
	 	if (filamentneeded >= filamentneeded && powerLevel >= powerneeds){
	 		filamentLevel -= filamentneeded;
	 		powerLevel -= powerneeds;
	 		objectsPrinted ++;
	...
	public void printStatus(){
		...
		System.out.println ("напечатало" + objectsPrinted);
		System.out.println ();
...
```diff
ну и нейминг у переменных. не дай Бог вы будете со мной работать где-то
3/4
```
